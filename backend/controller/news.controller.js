  ...
  if (pastSources && pastSources.length) {
    ...
    summaryParts.push(
      mostRecentStr
        ...
        : "No live incidents detected at this time."
    );
    ...
    pastSources.slice(0, 5).forEach((item, idx) => {
      ...
      if (date) summaryParts.push(`   Date: ${date}`);
      if (cleanSnippet) summaryParts.push(`   Details: ${cleanSnippet}...`);
      summaryParts.push("");
    });
...
const fetchPastIncidents = async (query, locationName) => {
  try {
    ...
    return (pastSearchResults.results || [])
